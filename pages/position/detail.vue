	...
	<joy-page class="">
		<view class="body">
			...
			<view class="user space-between">
				<view class="left">
					<image style="width: 100upx; height: 100upx" :src="position.userAvatar || '/static/img/head.png'"></image>
					<view class="info">
						<view class="name">
			...
			<view class="company">
				<view class="company-info space-between" @click="toCompany()">
					<view class="left">
						<image style="width: 100upx; height: 100upx" :src="position.companyLogo || '/static/img/company.png'"></image>
						<view class="info">
							<view class="name">
				...
				<view class="company-location">
					<map :latitude="latitude" :longitude="longitude" :markers="covers" style="width: 100%; height: 300px"></map>
				</view>
			</view>
			<view class="warn">
				<view class="top">
...
export default {
	...
	async onLoad(query) {
		...
		this.qqmapsdk = new QQMapWX({
			key: 'HHUBZ-HFJK3-IIT32-RVIXT-AMJE7-4HBYX',
		})
	...
	methods: {
		...
		async apply() {
			const that = this
			if (!this.isApply && this.position.userId !== this.userInfo.id) {
			...
			} else if (this.position.userId === this.userInfo.id) {
				uni.showToast({
					title: '不能投递自己的职位',
					duration: 2000,
				})
