	...
	<joy-page class="">
		<view class="mp-search">
			<view class="mp-search-input" @click="search">
				<text
					class="yzb yzb-search"
					style="font-size: 36upx; margin-right: 15upx"
				></text>
				<text>搜索岗位或关键词 ssssssssssss</text>
			</view>
		</view>
		<view style="position: relative; top: 80upx">
			...
			<view class="post">
				...
				<view class="post-list">
					...
					>
						<text
							class="post-item"
							:class="index % 4 != 0 ? 'post-item-left' : ''"
							>{{ item.name }}</text
						>
					</view>
				</view>
			...
			<template
				v-if="hasLogin == false || (hasLogin && userInfo.role == '求职者')"
			>
				<view class="expect">
					<scroll-view class="items" :scroll-x="true"
						><text class="title">热门工作</text></scroll-view
					>
				</view>
			...
			<template v-if="hasLogin && userInfo.role == '招聘者'">
				<view class="expect">
					<scroll-view class="items" :scroll-x="true"
						><text class="title">推荐简历</text></scroll-view
					>
				</view>
