	...
	<view>
		...
		<view class="orders">
			<view class="box" v-if="userInfo.role == undefined || userInfo.role == '求职者'">
				<view class="label" v-for="(row, index) in orderTypeList" :key="row.name" hover-class="hover" @tap="toOrderType(index)">
					<view class="icon">
			...
			<view class="box" v-else>
				<view class="label" v-for="(row, index) in orderTypeList2" :key="row.name" hover-class="hover" @tap="toOrderType2(index)">
					<view class="icon">
						<view class="badge" v-if="row.badge > 0">{{ row.badge }}</view>
						<text class="yzb label-icon" :class="row.icon"></text>
					</view>
		...
		<template v-if="userInfo.role == undefined || userInfo.role == '求职者'">
...
export default {
		...
		return {
			orderTypeList: [
				//name-标题 icon-图标 badge-角标
				{
					...
					icon: 'yzb-jianli',
					badge: 0,
					url: this.$mRoutesConfig.personResume,
				},
				{
					name: '我的投递',
					show: true,
					icon: 'yzb-mianshiyaoqing',
					badge: 0,
					url: this.$mRoutesConfig.apply,
				},
				{
					name: '我的收藏',
					show: true,
					icon: 'yzb-ziyuan141',
					badge: 0,
					url: this.$mRoutesConfig.collect,
				},
				{
					name: '浏览记录',
					show: true,
			...
			orderTypeList2: [
				//name-标题 icon-图标 badge-角标
				{
					...
					icon: 'yzb-liulanjilu',
					badge: 0,
					url: this.$mRoutesConfig.companyDetail,
				},
				{
					name: '面试邀请',
					show: true,
	...
	methods: {
		...
		//用户点击订单类型
		toOrderType(index) {
			console.log('用户点击订单类型', index)

			// uni.showToast({ title: this.orderTypeList[index].name });
			this.$mRouter.push({
				route: this.orderTypeList[index].url,
				query: {
					id: 1,
				},
			})
		...
		toOrderType2(index) {
			// uni.showToast({ title: this.orderTypeList[index].name });
			this.$mRouter.push({
				route: this.orderTypeList2[index].url,
				query: {
		...
		async switchRole(role) {
			...
			if (this.userInfo.companyId) {
				...
				if (role == 'DEFAULT') {
					//跳转到简历编辑页面。
					this.$mRouter.push({
						route: this.$mRoutesConfig.personResume,
						query: {
							id: 1,
						},
					})
