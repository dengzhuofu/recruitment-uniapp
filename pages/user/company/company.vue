<template>
	<view class="content reg-page">
		<view class="input-group">
			...
			<view class="input-row border-bottom-1px">
				...
				<view class="picker row align-width">
					<picker @change="industryChange" :value="industry" :range="industryArray" range-key="name">
						<input class="padding-10" style="text-align: right" placeholder="请选择公司性质" :value="company.industryName" />
					</picker>
					<text class="yzb yzb-next"></text>
				</view>
			</view>
			<view class="input-row border-bottom-1px">
				...
				<view class="picker row align-width">
					<picker @change="natureChange" :value="nature" :range="natureArray">
						<input class="padding-10" style="text-align: right" placeholder="请选择公司性质" :value="company.nature" />
					</picker>
					<text class="yzb yzb-next"></text>
				</view>
			</view>
			<view class="input-row border-bottom-1px">
				...
				<view class="picker row align-width">
					<picker @change="staffSizeChange" :value="staffSize" :range="staffSizeArray">
						<input class="padding-10" style="text-align: right" placeholder="请选择人员规模" :value="company.staffSize" />
					</picker>
					<text class="yzb yzb-next"></text>
				</view>
			</view>
			...
			<view class="input-column border-bottom-1px" style="margin-bottom: 100px"
				><text class="super-title" @tap="getLocation">公司位置</text>
				<map
					v-if="location"
...
export default {
	...
	data() {
		return {
			id: 0,
			location: null, // 预先声明 location 属性
			markers: [], // 预先声明 markers 属性
			qqmapsdk: null, // 声明 qqmapsdk 属性
			date1: '',
			date2: '',
	...
	async onLoad() {
		if (this.userInfo.companyId) {
			await this.getCompanyInfo(this.userInfo.companyId)
		} else {
			this.company = {
				address: '',
				industryName: '',
	...
	methods: {
		getLocation() {
			...
			if (this.company.longitude && this.company.latitude) {
				// 如果已有经纬度信息，直接进行逆地址解析
				this.reverseGeocode(this.company.latitude, this.company.longitude)
				console.log('11111111111111', this.company.latitude, this.company.longitude)
		...
		reverseGeocode(latitude, longitude) {
			this.qqmapsdk.reverseGeocoder({
				...
				success: (addressRes) => {
					console.log('当前位置的地址信息：', addressRes.result.address)
					this.location = {
						longitude: longitude,
						latitude: latitude,
					...
					this.markers = [
						{
							...
							latitude: latitude,
							longitude: longitude,
							title: addressRes.result.address,
							width: 30, // 设置标记的宽度
							height: 40, // 设置标记的高度
							callout: {
								// 添加 callout 属性
					...
					this.company.address = addressRes.result.address
				},
				fail: (err) => {
		...
		async getCompanyInfo(id) {
			...
			if (res) {
				...
				console.log('000', this.company.longitude, this.company.latitude)
				this.company = res
