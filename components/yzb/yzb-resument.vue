<template>
	<view class="m-position">
		<view class="item" v-for="(item, index) in list" :key="item.id" @click="onClick(item)">
			<view v-if="type == 0" class="job">
				...
				<view class="desc">
					...
					<view v-if="item.skill">
						<text v-for="(item2, index2) in item.skill.split(',')" :key="index2">{{ item2 }}</text>
					</view>
				</view>
				<view class="user">
					<view class="left">
			...
			<view v-else class="resume">
				<view class="title space-between">
					...
					<image class="right" v-if="item.avatar" :src="item.avatar"></image>
				...
				<view class="exp" v-if="item.companyName">
					<view class="exp-item" :class="item.ifRead ? 'exp-item-read' : ''">
						...
						<text>{{ item.companyName }} · {{ item.postName }}</text>
					</view>
				</view>
				<view class="position">
					<view class="left">
